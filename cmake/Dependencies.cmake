# ------------------------------------------------------------------------------
# Third-party dependencies, fetched and pinned by tag.
#
...
# build 3rd-party libraries statically, links to engine directly
...
# ------------------------------------------------------------------------------
#
# NOTE: GLAD is a special case for dependency management because unlike other
# dependencies where you have to fetch them online from GitHub or other
# repositories, GLAD does not have that and instead let developers fetch them
# via a download from a web service. Thus, it might seem like GLAD is part of
# the project, but really it's a dependency handled manually and should be
# treated like one (that means no modifying willy-nilly!!!).
...
# NOTE: Dear IMGUI is build-agnostic. That is, it lets users build its files
# using their own build system. Hence, only the required files are used to build
# Dear IMGUI.
...
target_link_libraries(dear_imgui PUBLIC glfw glad::glad)
...
# NOTE: stb_image is a header-only library, and since it doesn't have its own
# build system, a custom but simple build procedure was written to include
# stb_image as a library.
...
# NOTE: flecs has their own CMakeLists.txt that is intrusive to this project
# (like installing its own header files), thus a custom build procedure is
# written to work around this issue.
...
target_include_directories(flecs
    PUBLIC ${flecs_repo_SOURCE_DIR}/include
    PRIVATE ${flecs_repo_SOURCE_DIR}/src
)
